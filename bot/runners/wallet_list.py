...
def load_excluded():
    """Lädt ausgeschlossene Trade-IDs aus JSON."""
...
def save_excluded(excluded_set):
    """Speichert ausgeschlossene Trade-IDs."""
    Path(EXCLUDED_FILE).write_text(
        json.dumps(sorted(list(excluded_set)), indent=2),
        encoding='utf-8'
    )
...
def load_all_trades(db_path):
    """Lädt alle Trades mit ID für Ausschluss-Funktion."""
    if not Path(db_path).exists():
        return {}
    try:
        conn = connect(db_path)
        rows = conn.execute("""
            SELECT s.id, s.wallet, s.token, s.price_eur as exit_price,
                   ...
                   s.timestamp as sell_time, s.max_price_pct, s.min_price_pct,
                   s.session_id,
...
def compute_wallet_stats_excluding(trades_by_wallet, excluded):
    """
    Berechnet Wallet-Statistiken ohne ausgeschlossene Trades.
    Gibt dict {wallet: {wins, losses, total_pnl, wr, avg_pnl, ev}} zurück.
    """
    result = {}
    for wallet, trades in trades_by_wallet.items():
        ...
        if not active:
            result[wallet] = {'wins': 0, 'losses': 0, 'total_pnl': 0,
                               'wr': 0, 'avg_pnl': 0, 'ev': 0, 'n': 0}
            continue
        ...
        result[wallet] = {
            ...
            'wr':        round(wr * 100, 1),
            'avg_pnl':   round(total / len(active), 2),
            'ev':        round(ev, 2),
            'n':         len(active),
        }
    return result
...
def generate_html(stats, categories, obs_counts, all_trades, excluded, db_label):
    ...
    for w, cat in categories.items():
        if w not in all_wallets:
            all_wallets[w] = {
                'wallet': w, 'total_trades': 0, 'winning_trades': 0,
                'losing_trades': 0, 'total_pnl_eur': 0, 'avg_pnl_eur': 0,
                'win_rate': 0, 'confidence_score': 0.2,
                'strategy_label': 'UNKNOWN', 'dynamic_sl': None,
                'dynamic_tp': None, 'last_updated': '-'
            }

    wallet_list = list(all_wallets.values())

    # Berechne Live-Stats ohne ausgeschlossene Trades
    ...
    for w in wallet_list:
        ...
        conf   = w['confidence_score'] or 0.2
        ...
        cat_colors = {
            'ActiveWallet':    '#4ade80',
            'CandidateWallet': '#facc15',
            'ArchivedWallet':  '#888',
            'OwnWallet':       '#60a5fa',
        }
        label_colors = {
            'ASYMMETRIC': '#a78bfa', 'RUNNER': '#38bdf8',
            'SCALPER': '#4ade80', 'LOSS_MAKER': '#f87171',
            'MIXED': '#fb923c', 'OBSERVER': '#94a3b8', 'UNKNOWN': '#555',
        }
        cat_color   = cat_colors.get(cat, '#888')
        label_color = label_colors.get(label, '#555')
        conf_pct    = int(conf * 100)
        conf_color  = '#4ade80' if conf >= 0.6 else '#facc15' if conf >= 0.35 else '#f87171'
        ...
        rows_html += f"""
        <tr onclick="showDetail('{addr}')" style="cursor:pointer;" class="wallet-row" data-wallet="{addr}" data-cat="{cat}" data-label="{label}" data-conf="{conf}" data-pnl="{pnl_val}" data-wr="{wr_val}" data-trades="{n_val}">
            <td>
            ...
            <td>
                <div style="display:flex;align-items:center;gap:6px;">
                    ...
                    <span style="color:{conf_color};font-weight:600;">{conf:.2f}</span>
                </div>
            </td>
...
<div class="controls">
  ...
  <select onchange="sortTable(this.value)" id="sort-select">
...
<table id="wallet-table">
  <thead>
    <tr>
      ...
      <th onclick="sortTable('conf')">Confidence</th>
...
<div style="margin-top:10px;font-size:11px;color:#444;">* Werte ohne ausgeschlossene Trades berechnet</div>
</div>
...
const REASON_SHORT = {{
  'WALLET_SOLD':'Wallet sold','OBSERVER_STAGNATION':'Stagnation',
  'OBSERVER_MAX_HOLD':'Max hold','PRICE_UNAVAILABLE':'No price',
  'SESSION_ENDED':'Session end','STOP_LOSS':'Stop loss',
  'TAKE_PROFIT':'Take profit','INACTIVITY':'Inactivity',
...
function toggleExclude(tradeId, cb) {{
  ...
  if (cb.checked) {{
    EXCLUDED.add(tradeId);
  }} else {{
    EXCLUDED.delete(tradeId);
  }}
  saveExcluded();
  recomputeStats();
  updateDetailStats(currentWallet);
  updateExcludedBar();
  updateStatsBar();
}}
...
function saveExcluded() {{
  fetch('http://localhost:{PORT}/save_excluded', {{
    ...
    console.warn('Server nicht erreichbar – Ausschlüsse nur im RAM.');
  }});
}}

function recomputeStats() {{
  // Neu berechnen ohne ausgeschlossene Trades
  const newStats = {{}};
  for (const [wallet, trades] of Object.entries(ALL_TRADES)) {{
    const active = trades.filter(t =>
      !EXCLUDED.has(String(t.id)) &&
      t.reason !== 'SESSION_ENDED' && t.reason !== 'CRASH_RECOVERY' &&
      !t.price_missing
    );
    if (!active.length) {{
      newStats[wallet] = {{wins:0,losses:0,total_pnl:0,wr:0,avg_pnl:0,ev:0,n:0}};
      continue;
    }}
    const wins   = active.filter(t => (t.pnl_eur||0) > 0);
    const losses = active.filter(t => (t.pnl_eur||0) <= 0);
    const total  = active.reduce((s,t) => s+(t.pnl_eur||0), 0);
    const wr     = wins.length / active.length;
    ...
    const ev     = wr*avgW - (1-wr)*Math.abs(avgL);
    newStats[wallet] = {{
      wins: wins.length, losses: losses.length,
      total_pnl: Math.round(total*100)/100,
      wr: Math.round(wr*1000)/10,
      avg_pnl: Math.round(total/active.length*100)/100,
  ...
  document.querySelectorAll('.wallet-row').forEach(row => {{
    ...
    row.dataset.pnl    = pnl;
    row.dataset.wr     = wr;
    row.dataset.trades = n;
    row.dataset.ev     = ev;

    const wrEl  = row.querySelector('.dyn-wr');
    const pnlEl = row.querySelector('.dyn-pnl');
    const nEl   = row.querySelector('.dyn-trades');
    const evEl  = row.querySelector('.dyn-ev');
...
function updateExcludedBar() {{
  ...
  if (EXCLUDED.size > 0) {{
    ...
    bar.textContent   = `⚠ ${{EXCLUDED.size}} Trade(s) ausgeschlossen – Statistiken werden ohne diese berechnet.`;
  }} else {{
...
function showDetail(addr) {{
  currentWallet = addr;
  const trades  = ALL_TRADES[addr] || [];
  const cat     = CATEGORIES[addr] || 'Unknown';
  const obs     = OBS_COUNTS[addr] || 0;
  const ls      = LIVE_STATS[addr] || {{}};
...
function updateDetailStats(addr) {{
  ...
  if (cat === 'CandidateWallet') {{
    ...
    obsProgress = `<div class="meta-card" style="grid-column:1/-1;">
      <div class="label">Observer Fortschritt</div>
      <div style="display:flex;align-items:center;gap:10px;margin-top:6px;">
        ...
        <span style="color:#facc15;font-weight:700;">${{obs}}/20</span>
      </div>
    </div>`;
  }}
  ...
  let tradesHtml = '';
  if (!trades.length) {{
    tradesHtml = '<p style="color:#555;text-align:center;padding:20px;">Noch keine Trades.</p>';
  }} else {{
    ...
    const trows = trades.map((t, i) => {{
      const pnlV   = t.pnl_eur || 0;
      const excl   = EXCLUDED.has(String(t.id));
      ...
      const rowCls = excl ? 'trade-row excluded' : 'trade-row';
      return `<tr class="${{rowCls}}" id="trade-row-${{t.id}}">
        <td onclick="event.stopPropagation()">
          <input type="checkbox" class="exclude-cb"
            ${{excl ? 'checked' : ''}}
            onchange="toggleExclude(${{t.id}}, this)"
            title="Trade ausschließen">
        </td>
    ...
    tradesHtml = `
      <h3 style="font-size:12px;color:#555;text-transform:uppercase;margin-bottom:8px;">
        Trades (${{trades.length}}) ${{badge}}
      </h3>
      <table class="trades-table">
        <thead><tr>
          <th title="Ausschließen">✕</th>
          <th>#</th><th>Token</th><th>Entry EUR</th><th>Exit EUR</th>
  ...
  document.getElementById('panel-body').innerHTML = `
    <div class="meta-grid">
      <div class="meta-card">
        ...
        <div class="value" style="font-size:13px;">${{cat.replace('Wallet','')}}</div>
      </div>
      <div class="meta-card">
      ...
      <div class="meta-card">
        <div class="label">PnL (bereinigt)</div>
        <div class="value" style="color:${{pnlColor(pnl)}};font-size:15px;">${{fmtPnl(pnl)}}</div>
      </div>
      <div class="meta-card">
        <div class="label">EV / Trade</div>
        <div class="value" style="color:${{pnlColor(ev)}};font-size:15px;">${{(ev>=0?'+':'')+ev.toFixed(2)}} EUR</div>
      </div>
      <div class="meta-card">
        <div class="label">Trades (W/L)</div>
        <div class="value" style="font-size:14px;">${{n}} <span style="color:#555;font-size:12px;">(${{wins}}W/${{losses}}L)</span></div>
      </div>
      <div class="meta-card">
        <div class="label">Adresse</div>
        <div class="value" style="font-size:10px;font-family:monospace;word-break:break-all;">${{addr}}</div>
      </div>
      ${{obsProgress}}
    </div>
    ${{tradesHtml}}
...
function filterTable() {{
  ...
  document.querySelectorAll('.wallet-row').forEach(r => {{
    ...
    const rowCat   = (r.dataset.cat   || '').toLowerCase();
    const rowLabel = (r.dataset.label || '').toLowerCase();
    const addr     = r.dataset.wallet;

    // Prüfen ob Wallet vollständig ausgeschlossen (alle Trades excluded)
    const trades   = ALL_TRADES[addr] || [];
    const allExcl  = trades.length > 0 && trades.every(t => EXCLUDED.has(String(t.id)));

    const matchSearch = !search || text.includes(search);
    const matchCat    = !cat    || rowCat.includes(cat);
    const matchLabel  = !label  || rowLabel.includes(label);
    const matchExcl   = showExcl || !allExcl;

    const show = matchSearch && matchCat && matchLabel && matchExcl;
    r.style.display = show ? '' : 'none';
    if (show) visible++;

    // Zeile abdunkeln wenn Trades ausgeschlossen
    const wallet   = r.dataset.wallet;
    const wTrades  = ALL_TRADES[wallet] || [];
    const hasExcl  = wTrades.some(t => EXCLUDED.has(String(t.id)));
...
function sortTable(key) {{
  ...
  const curKey = tbody.dataset.sortKey;
  const curDir = parseInt(tbody.dataset.sortDir || '-1');
  const dir    = curKey === key ? curDir * -1 : -1;
...
function updateStatsBar() {{
  ...
  rows.forEach(r => {{
    ...
    if ((ls.ev || 0) !== 0) {{ totalEv += ls.ev; evCount++; }}
    const cat  = (r.dataset.cat || '').toLowerCase();
    if (cat.includes('active'))    active++;
    if (cat.includes('candidate')) candidates++;
  }});
  ...
  document.getElementById('stats-bar').innerHTML = `
    <div class="stat">
      ...
      <div class="stat-value" style="color:#ddd;">${{rows.length}}</div>
    </div>
    <div class="stat">
    ...
    <div class="stat">
      ...
      <div class="stat-value" style="color:#facc15;">${{candidates}}</div>
    </div>
    <div class="stat">
      <div class="stat-label">PnL gesamt*</div>
      <div class="stat-value" style="color:${{pnlColor(totalPnl)}};">${{totalPnl>=0?'+':''}}${{totalPnl.toFixed(2)}} EUR</div>
    </div>
    <div class="stat">
      <div class="stat-label">Ø EV / Trade*</div>
      <div class="stat-value" style="color:${{pnlColor(avgEv)}};font-size:16px;">${{avgEv>=0?'+':''}}${{avgEv.toFixed(2)}} EUR</div>
    </div>
    <div class="stat">
      <div class="stat-label">Ausgeschlossen</div>
      <div class="stat-value" style="color:${{EXCLUDED.size>0?'#fb923c':'#555'}};font-size:16px;">${{EXCLUDED.size}} Trades</div>
    </div>
  `;
}}
...
# ─────────────────────────────────────────────────────────────────────
# Lokaler Webserver (für Ausschluss-Speicherung)
# ─────────────────────────────────────────────────────────────────────
...
def run(args=None):
    ...
    print(f"[WalletList] {len(all_trades)} Wallets | {sum(len(v) for v in all_trades.values())} Trades | {len(excluded)} ausgeschlossen")
